...
async def fetchResults(username, config):
    async with aiohttp.ClientSession() as session:
        ...
        def render():
            ...
            table = Table(title="Enumerated Accounts")
    	
            # Add columns
            table.add_column("Username", style="cyan1")
            table.add_column("Percent Complete", style="green1")
            table.add_column("Sites Checked", style="red1")
            

            percent = int((completed / total_sites) * 100)
            
            # Add rows
            table.add_row(username, f"[green1]{percent}%[/green1]", f"{completed}/{total_sites}")
            return table
...
def verifyUsername(username, config, sitesToSearch=None, metadata_params=None):
    ...
    # Add columns
    table.add_column("Time Completed", style="green1")
    
    # Add rows
    table.add_row(f"{round(end_time - start_time, 1)} seconds")
